...
class MultiSectionViewController: UIViewController {
    ...
    let sectionData = [
        ...
        Section(title: "Horizontal Waterfall Layout", data: [Int](201...270), cellType: WaterfallCell.self, layout: { environment, dataCount in
            ...
            let config = WaterfallConfiguration(dataCount: dataCount, rowCount: 8, itemSpacing: 10, sectionVerticalSpacing: 16, itemWidthProvider: { CGFloat.random(in: 100...200)}, sectionHeight: .fractionalHeight(0.5), environment: environment)
            let layout: NSCollectionLayoutSection = .horizontalWaterfallSection(config: config)
            layout.addHeader()
            return layout
        }),
        
        Section(title: "Vertical Waterfall Layout", data: [Int](271...340), cellType: WaterfallCell.self, layout: { environment, dataCount in
